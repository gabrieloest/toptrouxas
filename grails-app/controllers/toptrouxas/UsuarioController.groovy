...
class UsuarioController {
	...
	def imagem = {
		def pessoa = Usuario.get( params.id )
		byte[] avatar = pessoa.avatar
	...
	def authenticate = {
		def user = Usuario.findByEmailAndSenha(params.email, params.senha.encodeAsSHA())
		if(user){
			session.user = user
			...
			//				to "${user.email}"
			//				subject "Bem vindo "
			//				body 'Bem vindo ao topTrouxas.com ${user.nome}?'
			//			}
